...
void ARMJIT_Memory::SigsegvHandler(int sig, siginfo_t* info, void* rawContext)
{
    ...
    // Only faults raised by JIT code touching fastmem are ours. Anything else (including faults
    // while no console is running) goes straight to the previous handler, so the system crash
    // reporter still produces a proper backtrace.
    if (NDS::Current)
    {
        FaultDescription desc {};
        u8* curArea = (u8*)(NDS::Current->CurCPU == 0 ? NDS::Current->JIT.Memory.FastMem9Start : NDS::Current->JIT.Memory.FastMem7Start);
...
#endif
    // On Android the region is already sized by ASharedMemory_create / ASHMEM_SET_SIZE,
    // and ashmem rejects ftruncate with EINVAL.
