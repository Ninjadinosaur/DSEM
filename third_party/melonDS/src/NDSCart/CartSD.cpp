...
{
    // The cart re-patches on every reset; once our driver is in place there is nothing to do.
    // (Checking this first avoids a spurious "won't fit" error, since the installed driver
    // reports a smaller reserved size than the original placeholder did.)
    if (memcmp(&binary[dldioffset+0x10], &patch[0x10], 0x30) == 0)
    {
        Log(LogLevel::Debug, "DLDI driver already applied at %08X\n", dldioffset);
        return;
    }
