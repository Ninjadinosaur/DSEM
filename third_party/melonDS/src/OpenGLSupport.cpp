...
#ifdef MELONDS_GLES
// OpenGL ES 3.2 port (Android): the shaders are written for desktop GLSL 1.40 / 4.30.
// Swap the #version line for ES, add the default precisions ES requires, and declare
// fragment output locations in the source (ES has no glBindFragDataLocation).
static std::string TranslateForGLES(const std::string& src, bool compute,
                                    const std::initializer_list<AttributeTarget>& fragOutputs)
{
    std::string body = src;
    if (body.compare(0, 8, "#version") == 0)
    {
        size_t eol = body.find('\n');
        body = (eol == std::string::npos) ? std::string() : body.substr(eol + 1);
    }

    for (const AttributeTarget& target : fragOutputs)
    {
        std::regex decl(std::string("\\bout\\s+(\\w+)\\s+") + target.Name + "\\s*;");
        body = std::regex_replace(body, decl,
            "layout(location = " + std::to_string(target.Location) + ") out $1 " + target.Name + ";");
    }
...
#ifndef MELONDS_GLES
    // Only meaningful with a dynamic GL loader; on ES the functions are linked directly.
    if (!glCreateShader)
    {
...
#ifdef MELONDS_GLES
    if (!CompilerShader(shader, TranslateForGLES(source, true, {}), name, "compute"))
        goto error;
#else
...
#ifdef MELONDS_GLES
    if (!CompilerShader(shaders[0], TranslateForGLES(vs, false, {}), name, "vertex"))
        goto error;

    if (!CompilerShader(shaders[1], TranslateForGLES(fs, false, fragmentOutAttrs), name, "fragment"))
        goto error;
#else
